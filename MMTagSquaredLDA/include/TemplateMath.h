...
public:
	...
	static int get_max_index_from_vector(std::vector<T> const & v, size_t limit);
};
 ...
 */
template<typename T>
int CTemplateVectorMath<T>::get_max_index_from_vector(std::vector<T> const & v, size_t limit)	{
	T mx = v[0];
	int index = 0;
	size_t length = v.size();
	if ( limit > (v.size() - 1) ) { std::cout << " limit is greater than vector size" << std::endl; return -1; }
	for (size_t i = 0; (i < length) && (i <= limit); ++i) {
		//if (b_verbose_global) std::cout << " " << v->at(i);
		if ( v.at(i) > mx ) {
			mx = v.at(i);
			index = i;
