/*
 * CCorrLDAEMFunctionoid.cpp
 *
 *  Created on: Oct 14, 2010
 *      Author: data
 */

#include "CCorrLDAEMFunctionoid.h"
#include "../Utilities/CDirichletOptimizer.h"
#include "../DataSet/CDictionaryReader.h"
using namespace std;

CCorrLDAEMFunctionoid::CCorrLDAEMFunctionoid() {
	_model = new CCorrLDA();
	_base_model = dynamic_cast<CCorrLDAGeneralModel*>(_model);
...
void CCorrLDAEMFunctionoid::operator ()() {
	////////////////////////////////////////////////////////////////
	...
	if ( _model->_operation_mode == "predict" ) {
		////// -- for prediction with datum count set to 0
	...
	if ( _model->get_model_operation_mode() == "train" ) {
		...
		pmr.print_topics(_model->_log_beta, index_file, 20, ofs);
 ...
 */
void CCorrLDAEMFunctionoid::expectation_step() {
	_model_log_likelihood = 0;

	SCorrLDADocument* doc;
	size_t d, k, n, m;
 ...
 * @brief expectation step for CorrLDA
 */
double CCorrLDAEMFunctionoid::doc_expectation_step(size_t doc_id, SCorrLDADocument* doc) {
	...
	while ( (converged > VAR_CONVERGED) &&
		...
		for (m = 0; m < doc->_num_of_datum; ++m) {
			...
			for (n = 0; n < doc->_num_of_corr_datum; ++n) {
				...
				(*(*_model->_var_lambda)[m])[n] = var_phi_sum;

				if ( n > 0 )
					var_lambda_sum =
							CMathUtilities::log_sum((*(*_model->_var_lambda)[m])[n], var_lambda_sum);
				else
			...
			for (n = 0; n < doc->_num_of_corr_datum; ++n) {
				...
				assert(!isnan((*(*_model->_var_lambda)[m])[n]));
			}
		}
		...
		for ( n = 0; n < doc->_num_of_corr_datum; ++n ) {
			...
			for ( k = 0 ; k < _model->_K; ++k ) {
				...
				(*(*_model->_var_phi)[n])[k] =
						(
								digamma_var_gamma[k]
								                  + (*(*_model->_log_rho)[k])[doc->_corr_datum_id_vec->at(n)]
						);

				var_lambda_sum = 0;
				// iteration over the M loop for phi
				for ( m = 0; m < doc->_num_of_datum; ++m )
					var_lambda_sum += (*(*_model->_var_lambda)[m])[n]
					                                                        * (*(*_model->_log_beta)[k])[ (*doc->_datum_id_vec)[m] ];

				(*(*_model->_var_phi)[n])[k] += var_lambda_sum;
 ...
 */
void CCorrLDAEMFunctionoid::maximization_step() {
	size_t k, j;
	...
	for (k = 0; k < _model->_K; ++k) {
		...
		for (j = 0; j < _model->_V; ++j) {
			...
			if ( (*(*_model->_beta_ss)[k])[j] > 0 ) {
				...
				//cout << "_beta_ss = " << m_model->_beta_ss->at(k)->at(j) << endl;
 ...
 */
void CCorrLDAEMFunctionoid::final_e_step() {
 ...
 */
void CCorrLDAEMFunctionoid::final_e_step_predict() {

	_model_log_likelihood = 0;

	SCorrLDADocument* doc;
	size_t d, k, n, m;
	double phi_nk, lambda_phi_sum;
	...
	string per_doc_predictions_filename = _model->get_model_root_dir() + CUtilities::_file_path_separator +
			"per_doc_predictions-CorrMMLDA-"+CUtilities::number_to_string<size_t>(_model->_K)+"-none-rewt.txt";
	cout << "Writing prediction strings to " << per_doc_predictions_filename << endl;
	...
	for ( d = 0; d < _model->_D; ++d ) {
		...
		for (k = 0; k < _model->_K; ++k) {
			...
			for (m = 0; m < doc->_num_of_datum; ++m) {
				...
				for ( n = 0; n < doc->_num_of_corr_datum; ++n )
					lambda_phi_sum += (*(*_model->_var_lambda)[m])[n] * (*(*_model->_var_phi)[n])[k];

				(*(*_model->_beta_ss)[k])[ (*doc->_datum_id_vec)[m] ] += (*doc->_datum_id_count_vec)[m] * lambda_phi_sum;

				(*_model->_beta_total_ss)[k] += (*doc->_datum_id_count_vec)[m] * lambda_phi_sum;
			}
		}
