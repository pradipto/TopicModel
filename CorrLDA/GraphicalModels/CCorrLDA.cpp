...
CCorrLDA::~CCorrLDA() {
	...
	if ( _asym_alpha_exp_square_ss ) { delete _asym_alpha_exp_square_ss; }
}

/**
 * @brief (re)initialize the sufficient statistics matrices and scalars to 0
 */
bool CCorrLDA::zero_initialize_suff_stats() {
 ...
 */
void CCorrLDA::reset_data_structures() {
 ...
 */
void CCorrLDA::read_data_from_file() {
 ...
 */
void CCorrLDA::read_data_from_file_for_training() {
	...
	if ( model_index_file_stream.is_open() )	{
		...
		while ( !model_index_file_stream.eof() ) {
			...
			// create a document
			SCorrLDADocument *doc = new SCorrLDADocument();
 ...
 */
void CCorrLDA::read_data_from_file_for_testing() {
 ...
 *
 */
bool CCorrLDA::create_and_initialize_data_structures() {
	bool status = true;
	if ( _operation_mode == "train" ) {
 ...
 */
bool CCorrLDA::create_and_initialize_data_structures_for_parameters() {
	bool status = false;
	/*
 ...
 */
bool CCorrLDA::create_and_initialize_data_structures_for_hidden_variables_and_suff_stats() {
	bool status = false;
	/*
 ...
 *
 */
void CCorrLDA::read_config_file(string model_config_filename) {
 ...
 */
void CCorrLDA::print_parameters() {
 ...
 */
void CCorrLDA::corpus_initialize()
{
	size_t i, k, d, n, m, j;
 ...
 * @brief initialize the corpus data structure with a seeded set of documents
 */
void CCorrLDA::random_initialize() {
 ...
 */
double CCorrLDA::compute_log_likelihood() {
 ...
 */
double CCorrLDA::compute_doc_log_likelihood(size_t doc_id, SCorrLDADocument* doc) {
	...
	double temp_sum, phi_nk, lambda_mn;
	for ( n = 0; n < doc->_num_of_corr_datum; ++n ) {
	...
	for ( m = 0; m < doc->_num_of_datum; ++m ) {
		...
		for ( n = 0; n < doc->_num_of_corr_datum; ++n ) {
			...
			if ( lambda_mn > 0 )
				log_likelihood -= lambda_mn * log(lambda_mn)
	...
	for ( m = 0; m < doc->_num_of_datum; ++m ) {
		...
		for ( k = 0; k < _K; ++k ) {
			...
			temp_sum = 0;
			for ( n = 0; n < doc->_num_of_corr_datum; ++n )
				temp_sum += (*(*_var_lambda)[m])[n] * (*(*_var_phi)[n])[k];

			log_likelihood +=  temp_sum * (*(*_log_beta)[k])[ (*doc->_datum_id_vec)[m] ]
			                                                  * (*doc->_datum_id_count_vec)[m];
		}
	}
