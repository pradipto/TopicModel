/*
 * CCorrTagSquaredLDAEMFunctionoid.cpp
 *
 *  Created on: Oct 14, 2010
 *      Author: data
 */

#include "CCorrTagSquaredLDAEMFunctionoid.h"
...
CCorrTagSquaredLDAEMFunctionoid::CCorrTagSquaredLDAEMFunctionoid() {
	_model = new CCorrTagSquaredLDA();
	_base_model = dynamic_cast<CCorrTagSquaredLDAGeneralModel*>(_model);
...
void CCorrTagSquaredLDAEMFunctionoid::operator ()() {
	////////////////////////////////////////////////////////////////
 ...
 */
void CCorrTagSquaredLDAEMFunctionoid::expectation_step() {

	SCorrTagSquaredLDADocument* doc;
	size_t d, k, n, m;
	double lambda_phi_sum;
		...
		for (k = 0; k < _model->_K; ++k) {
			...
			for (m = 0; m < doc->_num_of_datum; ++m) {
				...
				for ( n = 0; n < doc->_num_of_corr_datum; ++n )
					lambda_phi_sum += (*(*_model->_var_lambda)[m])[n]
					                                                        * (*(*_model->_var_phi)[n])[k];

				(*(*_model->_beta_ms_term1)[k])[ (*doc->_datum_id_vec)[m] ] +=
						(*doc->_datum_id_count_vec)[m] * lambda_phi_sum;

				(*(*_model->_beta_ms_term2)[k])[ (*doc->_datum_id_vec)[m] ] +=
						(*doc->_datum_id_count_vec)[m] *
						(
								(1.0/(*_model->_var_zeta_perdoc)[m]) * lambda_phi_sum *
		...
		for (k = 0; k < _model->_K; ++k) {
			...
			for (n = 0; n < doc->_num_of_corr_datum; ++n) {
				(*(*_model->_rho_ss)[k])[ (*doc->_corr_datum_id_vec)[n] ] +=
						(*doc->_corr_datum_id_count_vec)[n] * (*(*_model->_var_phi)[n])[k];
 ...
 */
double CCorrTagSquaredLDAEMFunctionoid::doc_expectation_step(size_t doc_id, SCorrTagSquaredLDADocument* doc) {
	...
	double var_phi_sum, var_lambda_sum, lambda_phi_sum;
	double var_phi_sum_term1, var_phi_sum_term2;
	double var_lambda_sum_term1, var_lambda_sum_term2;
	double var_zeta_sum;
	double phi_nk, lambda_mn;
	...
	/*
	 * initialize matrix var_lambda (M_max x N_max)
	 */
	for ( m = 0; m < doc->_num_of_datum; ++m )
		for ( n = 0; n < doc->_num_of_corr_datum; ++n )
			(*(*_model->_var_lambda)[m])[n] = 1.0/doc->_num_of_corr_datum;
	...
	while ( (converged > VAR_CONVERGED) &&
		...
		for (m = 0; m < doc->_num_of_datum; ++m) {
			...
			for (n = 0; n < doc->_num_of_corr_datum; ++n) {
				...
				for ( k = 0; k < _model->_K; ++k ) {
					...
					var_phi_sum_term1 += phi_nk
							* (
									(*(*_model->_log_beta)[k])[ (*doc->_datum_id_vec)[m] ]
									                                     + (*(*_model->_log_pi)[ (*doc->_tag_id_vec)[m] ])[ (*doc->_datum_id_vec)[m] ]
							);

					var_phi_sum_term2 += phi_nk
							* (*(*_model->_beta_pi_sum_mat)[k])[ (*doc->_tag_id_vec)[m] ];

				}

				(*(*_model->_var_lambda)[m])[n] = var_phi_sum_term1 -
						(1.0/(*_model->_var_zeta_perdoc)[m]) * var_phi_sum_term2;

				if ( n > 0 )
					var_lambda_sum =
							CMathUtilities::log_sum(var_lambda_sum, (*(*_model->_var_lambda)[m])[n]);
				else
					var_lambda_sum = (*(*_model->_var_lambda)[m])[n];

			}

			for ( n = 0; n < doc->_num_of_corr_datum; ++n ) {

				(*(*_model->_var_lambda)[m])[n] = exp( (*(*_model->_var_lambda)[m])[n] - var_lambda_sum );
		 ...
		 * 				\gamma_k^{t+1} = \alpha_k + \phi_{nk}^{t+1((*(*_model->_var_phi)[n])[k])}
		...
		for ( n = 0; n < doc->_num_of_corr_datum; ++n ) {
			...
			for ( k = 0 ; k < _model->_K; ++k ) {
				...
				var_lambda_sum_term1 = 0.0;
				var_lambda_sum_term2 = 0.0;

				// iteration over the M (datum) loop for phi
				for ( m = 0; m < doc->_num_of_datum; ++m ) {
					lambda_mn = (*(*_model->_var_lambda)[m])[n];
					var_lambda_sum_term1 += lambda_mn
							* (
									(*(*_model->_log_beta)[k])[ (*doc->_datum_id_vec)[m] ]
									                                     + (*(*_model->_log_pi)[ (*doc->_tag_id_vec)[m] ])[ (*doc->_datum_id_vec)[m] ]
							);
					...
					//for ( m = 0; m < doc->_num_of_datum; ++m )
					var_lambda_sum_term2 += lambda_mn
							* (1.0/(*_model->_var_zeta_perdoc)[m])
							* (*(*_model->_beta_pi_sum_mat)[k])[ (*doc->_tag_id_vec)[m] ];
				}

				(*(*_model->_var_phi)[n])[k] += var_lambda_sum_term1 - var_lambda_sum_term2;
...
/**
 * @brief maximization step for CorrTagSquaredLDA
 */
void CCorrTagSquaredLDAEMFunctionoid::maximization_step() {
	size_t k, j, t;
...
void CCorrTagSquaredLDAEMFunctionoid::final_e_step() {
	SCorrTagSquaredLDADocument* doc;
	size_t d, k, n, m;
	double lambda_phi_sum;
	...
	for ( d = 0; d < _model->_D; ++d ) {
		...
		vector<double> lambda_m_k_vec;
		for ( m = 0; m < doc->_num_of_datum; ++m ) {
			lambda_m_k_vec.clear();
			for ( k = 0; k < _model->_K ; ++k ) {
				double lambda_sum = 0;
				for ( n = 0; n < doc->_num_of_corr_datum; ++n ) {
					lambda_sum += (*(*phi)[n])[k]*(*(*lambda)[m])[n];
				}
				lambda_m_k_vec.push_back(lambda_sum);
			}
			assert(lambda_m_k_vec.size() == _model->_K);
			term_topic = CTemplateVectorMath<double>::get_max_index_from_vector(lambda_m_k_vec);
			ofs_per_doc_assignments << " " << (*doc->_datum_id_vec)[m] << ":"  << term_topic << ":";
			for ( size_t i = 0; i < _model->_K; ++i )
				ofs_per_doc_assignments << lambda_m_k_vec[i] << ",";
		}
		...
		for (k = 0; k < _model->_K; ++k) {
			...
			for (m = 0; m < doc->_num_of_datum; ++m) {
				...
				temp_ms_term1 = (*doc->_datum_id_count_vec)[m] * lambda_phi_sum;
